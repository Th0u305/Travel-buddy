          ...
          {travelList?.tags && travelList.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-8">
              {travelList.tags.map((tag: string, i: number) => (
                <span
                  key={i}
                  className="px-3 py-1 md:px-4 md:py-1.5 bg-accent text-accent-foreground text-xs uppercase font-bold tracking-wider rounded-md"
                >
                  #{tag.trim()}
                </span>
              ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-5 gap-12">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-surface-container-low rounded-4xl p-8 bg-[#f3f4ee]">
            ...
            <Button
              variant="link"
              className="w-full mt-6 flex items-center justify-center gap-2 text-primary font-bold hover:underline"
            >
