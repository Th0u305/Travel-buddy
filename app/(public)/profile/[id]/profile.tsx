...
import { useGetFullProfile, useGetTravelListById } from '@/src/tanstack/useQuery';
...
export default function Profile({id} : {id : string}) {
  ...
  const { profile, isLoading } = useGetFullProfile(id);
  ...
  const visitedCountries = profile?.visited_countries[0]
  const countriesLength = visitedCountries ? visitedCountries?.split(',').map((c: string) => c.trim()).filter(Boolean)?.length : 0
  const travelPlans = Array.isArray(profile?.travel_plans) ? profile.travel_plans : [];
  const startDate = new Date(profile?.travel_plans[0]?.start_date);
              ...
              <div className="relative">
                <div className="w-32 h-32 md:w-48 md:h-48 rounded-xl border-4 border-background overflow-hidden shadow-xl bg-background">
                  <Image 
                    ...
                    className="w-full h-full object-cover" 
                    src={profile.profile_picture || profile.avatar_url || fallbackProfilePic}
                  />
                </div>
              </div>
              
              <div className="flex-1 mb-2">
                <h1 className="text-white text-4xl md:text-5xl font-extrabold tracking-tight mb-2">{profile.full_name}</h1>
                <div className="flex items-center gap-3 text-white/90 font-medium">
                  ...
                  <span className="text-lg">{travelPlans.length} Expeditions</span>
                </div>
              </div>
            ...
            <div className="lg:col-span-4 space-y-12">
              <Card className="bg-muted/30 border-none shadow-sm">
                <CardContent className="p-8">
                  <h3 className="text-xl font-bold mb-4">About {profile?.full_name}</h3>
              ...
              <Card className="bg-muted/50 border-none shadow-sm">
                <CardContent className="p-8">
                  ...
                  <div className="grid grid-cols-2 gap-6">
                      <div>
                        <div className="text-orange-600 text-3xl font-bold">{countriesLength}</div>
                        <div className="text-muted-foreground text-xs uppercase font-bold tracking-widest mt-1">Countries Visited</div>
                      </div>
                      <div>
                        <div className="text-orange-600 text-3xl font-bold">{travelPlans.length}</div>
                        <div className="text-muted-foreground text-xs uppercase font-bold tracking-widest mt-1">Trips Planned</div>
                      </div>
                  </div>
                </CardContent>
              </Card>
              ...
              <section>
                ...
                {travelPlans.length > 0 ? (
                  <div className="space-y-6">
                    {travelPlans.map((trip: TravelBuddiesTs, i: number) => {

                      
                      return (
                      <div key={i} className="group relative flex flex-col md:flex-row bg-card rounded-xl overflow-hidden shadow-sm hover:shadow-md border transition-shadow duration-300">
                   
                          <Image
                            width={500}
                            height={500}
                            alt={trip.title} 
                            className={`w-60 h-60 object-cover transition-all duration-500 group-hover:scale-105 ${!isUpcoming ? 'grayscale group-hover:grayscale-0' : ''}`} 
                        ...
                        <div className="flex-1 p-6 flex flex-col justify-between">
                          <div>
                            <div className="flex justify-between items-start mb-2">
                              <Badge className={`${isUpcoming ? 'bg-orange-100 text-orange-700 hover:bg-orange-100' : 'bg-blue-100 text-blue-700 hover:bg-blue-100'} px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border-none`}>
                            ...
                            <p className="text-muted-foreground text-sm line-clamp-2">
                               ...
                               {trip.tags && trip.tags.length > 0 ? ` • ${trip.tags.join(', ')}` : ''}
                            </p>
                          </div>
                          
                          <div className="mt-4 flex items-center justify-between">
                            <Badge size="lg" variant="secondary">{trip.travel_type}</Badge>
                            
                            <Button onMouseEnter={() => setTripSlug(trip.slug)} variant={"outline"} className={`font-bold text-sm rounded-full`}>
                              <Link  prefetch={true} href={`/trips/${trip.slug}`}>
                                Details
                              </Link>
                            </Button>
                          </div>
